import React from 'react'
import { Link } from 'react-router-dom'
import AddToWatchList from '../components/AddToWatchList';
import { useSelector, useDispatch } from 'react-redux';
import RemoveFromWatchList from './RemoveFromWatchList';
import { updateAnimesStart, updateAnimesSuccess, updateAnimesFailure } from '../redux/animes/animeSlice.js'

const AnimeCard = ({ anime, disableLink = false, isDeleteAnimePage = false }) => {

    
    const { currentUser } = useSelector(state => state.user);

    const watchList = currentUser?.watchList;

    const dispatch = useDispatch();

    const handleDeleteAnime = async (event) => {
        event.preventDefault();

        try {

            dispatch(updateAnimesStart());
            
            const res = await fetch('/api/anime/deleteanime', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ animeId: anime._id})
            })

            const data = await res.json();

            if(data.success === false){
                dispatch(updateAnimesFailure(data.message));
                return;
            }

            dispatch(updateAnimesSuccess(data));

        } catch (error) {
            dispatch(updateAnimesFailure(error.message));
        }
    }

    return (
        <>
            {disableLink ? 
                <div className='anime-card'>
                    <div className="overlay"></div>
                    <img src={anime.coverImage} alt="anime-cover" />
                    <div className="anime-card-content">
                        <h3>{anime.title}</h3>
                        {currentUser && (watchList.includes(anime._id) ?
                            <RemoveFromWatchList animeId={anime._id} /> : <AddToWatchList animeId={anime._id}/>)
                        } 
                    </div>
                </div>
                 :
                 <Link to={`/animes/${anime._id}`} state={{ anime }} className='anime-card'>
                    <div className="overlay"></div>
                    <img src={anime.coverImage} alt="anime-cover" />
                    <div className="anime-card-content">
                    <h3>{anime.title}</h3>
                    {!isDeleteAnimePage && currentUser && (watchList.includes(anime._id) ?
                            <RemoveFromWatchList animeId={anime._id} /> : <AddToWatchList animeId={anime._id}/>)
                    }
                    {isDeleteAnimePage &&
                    <div onClick={handleDeleteAnime} className="add-icon delete-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" width={30}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                        </svg>
                    </div>   
                    } 
                    </div>
                </Link>
            }
        </>
    )
}

export default AnimeCard
